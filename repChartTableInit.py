...
def getCategories(cat):
     ...
     cur = con.cursor()  
     cur.execute("SELECT reqId, reqDesc, ReqCatType from Requirements where reqInit=?",(cat,))  
...
def getRequirements():
     con = sqlite3.connect("requirements.db") 
     cur = con.cursor()  
     cur.execute("SELECT reqInit, count(*) from Requirements group by reqInit")  
     userall =  cur.fetchall()
     con.commit()
     con.close()
     return userall
...
def TableReport():
  ...
  if t3.active:  
    ...
    for i in range(0,len(catDesc)):
      with hd.scope(i):
        hd.markdown("### Initiative: "+catDesc[i])
