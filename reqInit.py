...
def deleteReqInitiative(id):
     con = sqlite3.connect("requirements.db") 
     cur = con.cursor()
     try:
        sql = "DELETE FROM reqInitiative where Inititative = ?"
        cur.execute(sql, (id,))  
...
def getInitiatives():
     con = sqlite3.connect("requirements.db") 
     cur = con.cursor()  
     cur.execute("SELECT Inititative,InitDesc from reqInitiative")  
...
def insertreqInitiative(Initiative, InitDesc):
     con = sqlite3.connect("requirements.db") 
     cur = con.cursor()
     try:
        sql = "INSERT INTO reqInitiative VALUES( ?,?)"
        cur.execute(sql, (Initiative, InitDesc ))  
     except sqlite3.OperationalError as e:
          print(e) 
     finally:       
        con.commit()
...
def UpdateInitiative(Initiative, InitDesc):
     print(Initiative)
     print(InitDesc)
     con = sqlite3.connect("requirements.db") 
     cur = con.cursor()
     try:
        sql = "Update reqInitiative  set InitDesc = ? WHERE Inititative = ?"
        cur.execute(sql, (InitDesc, Initiative,))  
     except sqlite3.OperationalError as e:
          print(e) 
     finally:       
        con.commit()
              ...
              with hd.form( ) as form:    
                        ...
                        if form.submitted:  
                            if len(ErrorArray) == 0:   
                                ...
                                #print(listvalues[0][index])         
                                UpdateInitiative(listvalues[0][index], listvalues[1][index] )         
...
def addTable(dict, perpage,action,Subject,addtype,placement,types):
    ...
    listvalues.clear()
    listkeys.clear()
    tempvaluesInitiative = []
    tempvaluesInitTypeDesc = []
    ...
    x= getInitiatives()
    for rows in x: 
         tempvaluesInitiative.append(rows [0]) 
         datatabledict.update({"Initiative":tempvaluesInitiative}) 
         tempvaluesInitTypeDesc.append(rows [1]) 
         datatabledict.update({"Init Desc":tempvaluesInitTypeDesc}) 
                      ...
                      if len(ErrorArray) == 0:
                            ...
                            for i in range(1, len(listkeys)):
                                with hd.scope(i):
                                    ...
                                    datatabledict.update({listkeys[i]:listvalues[i]}) 
                            insertreqInitiative(form.form_data[listkeys[0]], form.form_data[listkeys[1]])
