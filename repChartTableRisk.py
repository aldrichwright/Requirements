...
def getCategories(cat):
     ...
     cur = con.cursor()  
     cur.execute("SELECT reqId, reqDesc, ReqCatType from Requirements where reqRiskType=?",(cat,))  
...
def getRequirements():
     con = sqlite3.connect("requirements.db") 
     cur = con.cursor()  
     cur.execute("SELECT reqRiskType, count(*) from Requirements group by reqRiskType")  
     userall =  cur.fetchall()
     con.commit()
     con.close()
     return userall
...
def TableReport():
  ...
  if t3.active:  
    ...
    for i in range(0,len(catDesc)):
      with hd.scope(i):
        hd.markdown("### Risk Type: "+catDesc[i])
        ...
        hd.data_table (dict(Id=(idArray),Description=(idDesc),Category=(idPrior)))
