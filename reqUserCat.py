...
def deleteUserCat(id):
     con = sqlite3.connect("requirements.db") 
     cur = con.cursor()
     try:
        sql = "DELETE FROM ReqUserCat where userCat = ?"
        cur.execute(sql, (id,))  
...
def getUserCats():
     con = sqlite3.connect("requirements.db") 
     cur = con.cursor()  
     cur.execute("SELECT userCat, reqCatDesc from reqUserCat")  
...
def insertUserCat(UserCat, reqCatDesc):
     con = sqlite3.connect("requirements.db") 
     cur = con.cursor()
     try:
        sql = "INSERT INTO reqUserCat VALUES( ?,?)"
        cur.execute(sql, (UserCat, reqCatDesc ))  
     except sqlite3.OperationalError as e:
          print(e) 
     finally:       
        con.commit()
...
def updateUserCat(UserCat, UserCatDesc):

     con = sqlite3.connect("requirements.db") 
     cur = con.cursor()
     try:
        sql = "Update reqUserCat  set reqCatDesc = ? WHERE UserCat = ?"
        cur.execute(sql, (UserCatDesc, UserCat,))  
     except sqlite3.OperationalError as e:
          print(e) 
     finally:       
        con.commit()
              ...
              with hd.form( ) as form:    
                        ...
                        if form.submitted:  
                            if len(ErrorArray) == 0:   
                                ...
                                #print(listvalues[0][index])         
                                updateUserCat(listvalues[0][index], listvalues[1][index] )         
...
def addTable(dict, perpage,action,Subject,addtype,placement,types):
    ...
    x= getUserCats()
    for rows in x: 
         tempvaluesUserCat.append(rows [0]) 
         datatabledict.update({"UserCat":tempvaluesUserCat}) 
         tempvaluesUserCatDesc.append(rows [1]) 
         datatabledict.update({"UserCatDesc":tempvaluesUserCatDesc}) 
                      ...
                      if len(ErrorArray) == 0:
                            ...
                            for i in range(1, len(listkeys)):
                                with hd.scope(i):
                                    ...
                                    datatabledict.update({listkeys[i]:listvalues[i]}) 
                            insertUserCat(form.form_data[listkeys[0]], form.form_data[listkeys[1]])
