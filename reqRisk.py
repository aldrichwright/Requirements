...
def deleteRisk(id):
     ...
     cur = con.cursor()
     try:
        sql = "DELETE FROM reqRisk where Risk = ?"
        cur.execute(sql, (id,))  
...
def getPriorities():
     con = sqlite3.connect("requirements.db") 
     cur = con.cursor()  
     cur.execute("SELECT Risk,RiskDesc from reqRisk")  
...
def insertreqRisk(Risk, RiskDesc):
     con = sqlite3.connect("requirements.db") 
     cur = con.cursor()
     try:
        sql = "INSERT INTO reqRisk VALUES( ?,?)"
        cur.execute(sql, (Risk, RiskDesc ))  
     except sqlite3.OperationalError as e:
          print(e) 
     finally:       
        con.commit()
...
def updatereqRisk(Risk, RiskDesc):

     con = sqlite3.connect("requirements.db") 
     cur = con.cursor()
     try:
        sql = "Update reqRisk  set RiskDesc = ? WHERE Risk = ?"
        cur.execute(sql, (RiskDesc, Risk,))  
     except sqlite3.OperationalError as e:
          print(e) 
     finally:       
        con.commit()
              ...
              with hd.form( ) as form:    
                        ...
                        if form.submitted:  
                            if len(ErrorArray) == 0:   
                                ...
                                #print(listvalues[0][index])         
                                updatereqRisk(listvalues[0][index], listvalues[1][index] )         
...
def addTable(dict, perpage,action,Subject,addtype,placement,types):
    ...
    for rows in x: 
         ...
         datatabledict.update({"Risk":tempvaluesCatType}) 
         tempvaluesCatTypeDesc.append(rows [1]) 
         datatabledict.update({"Risk Desc":tempvaluesCatTypeDesc}) 
                      ...
                      if len(ErrorArray) == 0:
                            ...
                            for i in range(1, len(listkeys)):
                                with hd.scope(i):
                                    ...
                                    datatabledict.update({listkeys[i]:listvalues[i]}) 
                            insertreqRisk(form.form_data[listkeys[0]], form.form_data[listkeys[1]])
